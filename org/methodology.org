...
This chapter bridges the theoretical framework established in Chapter 3 and the formal analysis to be presented in the next chapters. It presents the conceptual framework that operationalises the theoretical concepts for IBC specifically, followed by a step-by-step methodology for conducting the formal specification, LTS extraction, and behavioural comparison.
...
The theoretical framework in Chapter 3 introduced general concepts from process algebra and LTS theory. This subsection defines those concepts specifically for the IBC protocol.

*** Observable Behaviour in IBC

For the purposes of this research, \textbf{observable behaviour} of IBC consists of actions that are visible to an external observer of the protocol. The following actions are considered observable:
...
The following are considered \textbf{internal (silent) actions} and are abstracted as \(\tau\):
\begin{itemize}
...
*** Trace Definition for IBC

A \textbf{trace} in IBC is a finite sequence of observable actions from the initial channel establishment to either successful termination or deadlock. For example, a successful data transfer trace is:
...
*** Branching Structure in IBC

Branching structure occurs when the protocol has a choice between two or more continuations from a given state. In IBC, branching occurs at:
...
A \textbf{deadlock} in IBC is a state from which no further progress is possible but which is not a successful termination. Examples include:
...
These deadlock states are captured in the LTS as states with no outgoing transitions that are not marked as termination states (\(\checkmark\)).

** Research Paradigm: Formal Modelling as Inquiry

This research adopts a \textbf{formal modelling paradigm} rather than an empirical one. The following distinctions are critical:
...
*** The Three Analytical Scenarios (Unified, Stacked, Conflicting)

The central question of this research is whether IBC's asset-transfer and data-transfer behaviours are Unified, Stacked, or Conflicting. Table 4.1 provides the operational definitions for each scenario using the theoretical framework from Chapter 3.
...
Figure 4.1 illustrates the overall conceptual framework, showing the flow from IBC standards to the final classification.

#+begin_comment
#+CAPTION: Conceptual Framework for IBC Behavioural Analysis
#+NAME: fig:conceptual-framework
#+ATTR_LATEX: :width 0.9\textwidth
#+begin_src plantuml :file ../figures/conceptual-framework.png :exports results
@startuml
...
rectangle "IBC Specification Documents\n\n- ICS-004 (Data Transport)\n- ICS-020 (Asset Transfer)" as A

rectangle "Domain Analysis\n\n- Identify atomic actions\n  (send, receive, ack,\n   timeout, handshake)\n- Define alphabet:\n  Act = Act_ICS004 ∪ Act_ICS020\n- Define communication function γ" as B

rectangle "Process Algebra Specification\n\n- P_ICS004\n  (data transport with handshake,\n   packet flow, timeout)\n- P_ICS020\n  (asset transfer with lock,\n   mint, rollback)\n- Composition:\n  P_asset || P_data" as C

rectangle "LTS Extraction (via SOS rules)\n\n- States: S\n  (all reachable configurations)\n- Transitions:\n  labelled atomic actions\n- Termination: ✔\n- Deadlock: δ" as D

rectangle "Behavioural Analysis\n\n- Trace sets:\n  Traces(P_ICS004),\n  Traces(P_ICS020)\n- Branching structure preserved\n- Deadlock detection\n- Synchronisation patterns" as E

rectangle "Weak Bisimulation Check\n\n- P_ICS004 ≈ P_ICS020 ?\n- Trace inclusion?\n- New deadlocks in composition?" as F

rectangle "Classification\n\n→ UNIFIED (bisimilar)\n→ STACKED (trace inclusion)\n→ CONFLICTING (new deadlocks)" as G
...
#+RESULTS: fig:conceptual-framework
[[file:../figures/conceptual-framework.png]]
#+end_comment
...
This research will employ a \textbf{formal specification and analytical verification} design. The methodology follows a top-down decomposition approach:
...
*** Phase 1: Domain Analysis and Atomic Action Identification

This phase addresses \textbf{Objective 1} and provides the foundation for \textbf{RQ1}.
...
Sources: Cosmos IBC documentation and specification repositories [cite:@cosmos2024ibcintro] [cite:@cosmos_ibc].
...
Define the communication function \(\gamma\) that pairs send and read actions:
...
Mismatched or isolated actions (send without matching read, or vice versa) are blocked by the encapsulation operator \(\partial_H\).
...
*** Phase 2: Process Algebra Specification
...
\]
where \((PacketFlow)^*\) denotes zero or more repetitions of PacketFlow, and \(Close\) terminates the channel.
...
\]
where \(Rollback\) handles timeout or verification failure by unlocking the asset.
...
To test for conflicts, define the parallel composition of asset and data flows over the same channels:
\[
...
All specifications follow the syntax defined in Chapter 3 and will be included in the appendices.

*** Phase 3: LTS Extraction

This phase addresses \textbf{Objective 3} and \textbf{RQ2}.

**** Generate LTS from Process Terms

Using the SOS rules defined in Chapter 3 (Section 3.6), generate the LTS for each process term:
...
For each LTS (\(LTS_{Data}\), \(LTS_{Asset}\), \(LTS_{Combined}\)), record:
...
Produce transition graphs for each LTS. Example structure for \(LTS_{Data}\):
...
For each specification (\(P_{ICS004}\), \(P_{ICS020}\), \(P_{Combined}\)):
\begin{itemize}
...
*** Phase 4: Behavioural Comparison

This phase addresses \textbf{Objectives 4 and 5} and answers \textbf{RQ3}.
...
Apply the Cones and Foci methodology \cite{Fokkink2004} to determine if \(P_{ICS004} \approx P_{ICS020}\):
...
If \(\phi\) exists and all proof obligations are satisfied, then \(P_{ICS004} \approx P_{ICS020}\) (weak bisimilar).
...
If YES, then new deadlocks appear only when asset and data behaviours run concurrently. For each new deadlock, document the trace that leads to it.
...
\begin{verbatim}
                    ...
                    ▼                           ▼
            ┌───────────┐              ┌─────────────────┐
            │  UNIFIED  │              │ T_data ⊆ T_asset?│
            └───────────┘              └────────┬───┬────┘
                                                │   │
                                    ┌───────────┴───┴────────────┐
                                    │ YES (proper subset)        │ NO (or incomparable)
                                    ▼                            ▼
                            ┌───────────┐          ┌────────────────┐
                            │  STACKED  │          │ Check D_combined│
                            └───────────┘          │ for new deadlocks│
                                                   └────────┬───┬────┘
                                                            │   │
                                                ┌───────────┴───┴───────────┐
                                                │ YES (new deadlocks)       │ NO
                                                ▼                           ▼
                                        ┌──────────────┐        ┌──────────────┐
                                        ...
                                        └──────────────┘        │ (Re-examine) │
                                                                └──────────────┘
\end{verbatim}
\caption{Decision Tree for Unified/Stacked/Conflicting Classification}
...
List of all abstractions made (from Scope and Limitations in Chapter 1):
...
Unlike empirical research, formal modelling research requires different validation criteria.

*** Correctness of Formalisation
...
*** Completeness of LTS Extraction
...
This research involves no human subjects, no personal data collection, and no live blockchain interaction. The formal modelling and analysis are conducted entirely on publicly available protocol specifications (ICS-004, ICS-020). No economic or security risks are introduced as no actual assets or live transactions are involved. Proper attribution is given to IBC standard authors, process algebra pioneers, and all cited researchers.

** Summary

This chapter has presented the conceptual framework and methodology for analysing IBC's asset and data behaviours. The key elements are:
